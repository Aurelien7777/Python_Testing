...
def showSummary():
    ...
    return render_template(
        "welcome.html", club=club[0], competitions=competitions, clubs=clubs
    )
...
def purchasePlaces():
    ...
    competition_list = [c for c in competitions if c["name"] == competition_name]

    if not competition_list:
        flash("Sorry, competition was not found")
        return redirect(url_for("index"))

    club_list = [c for c in clubs if c["name"] == club_name]
    ...
    if not has_enough_competition_places(placesRequired, places_competition):
    ...
    competition["numberOfPlaces"] = calculate_remaining_places(
        places_competition, placesRequired
    )

    club["points"] = calculate_remaining_points(point_club, placesRequired)
